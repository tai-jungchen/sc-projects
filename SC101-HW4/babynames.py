...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        # same name in same year with two rank
        if year in name_data[name]:
            rank = min(int(rank), int(name_data[name][year]))
        name_data[name][year] = str(rank)
    else:
        # new name
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            lst = line.split(',')
            if len(lst) == 1:    # first line is year
                year = line.strip()
            else:
                rank, boy_name, girl_name = lst[0].strip(), lst[1].strip(), lst[2].strip()
